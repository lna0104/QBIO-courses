...
def bpay_payment(amount: int, from_account: int, bpay_payee: str) -> int:
    """Transfer money between two accounts.

    Parameters:
        amount: Amount to pay the BPay payee.
        from_account: Account from which funds will be debited.
        bpay_payee: Identifier of the BPay payee.

    Return: Updated balance of the from_account.
    """
    # Code to look up BPay payee's account from their BPay code.
    bpay_payee_account = 0
    if amount <= from_account:
        bpay_payee_account += amount
        from_account -= amount
    return from_account
...
while user_choice != QUIT :
    ...
    # Make a BPay payment.
        ...
        if from_account == '1' :
            ...
            amount = int(amount)
            savings = bpay_payment(amount, savings, bpay_payee)
        elif from_account == '2' :
            amount = input("Please enter the amount, in cents, to pay: ")
            amount = int(amount)
            high_interest = bpay_payment(amount, high_interest, bpay_payee)
        else :
    ...
    # Transfer money between the user's accounts.        
        ...
        print("\nSelect the account from which you wish to transfer money")
        from_account = input("  1) Savings,  2) High Interest > ")
        if from_account == '1' :
            print("Select the account to which you wish to transfer money")
